...
const Config = (function(){
  ...
  function renderSaveBar(){
    ...
    if(!RT.serverMode){
      right='<span class="sb-hint">read-only — run <code style="color:var(--accent)">rivet config --web</code> to edit</span><button class="sbtn save" disabled>🔒 Save</button>';
    } else {
  ...
  function doSave(){
    ...
    errorMap={};
    if(RT.gateLocked && window.RIVET.meta.inFlightTasks.length){ banners.gate=true; renderBanners(); Rivet.toast('🛡 Save refused — GATE-PROTECT-01','warn'); return; }
    manifest().forEach(k=>loaded[k.path]=clone(state[k.path]));
    RT.lastSynced=new Date(); banners.disk=false; renderBanners(); Rivet.renderSync(); after();
  ...
  function renderBanners(){
    ...
    let html='';
    if(!RT.serverMode) html+='<div class="banner ro"><span class="b-ic">🔒</span><div class="b-msg"><b>Read-only mode</b> — opened from disk with no server.<span class="sub">Run <span class="cmd">rivet config --web</span> to start the local editor and enable Save.</span></div></div>';
    ...
    box.querySelector('[data-bn="unlock"]')?.addEventListener('click',()=>{ RT.gateLocked=false; banners.gate=false; renderBanners(); renderSaveBar(); Rivet.toast('🔓 Lock released for 30 min — Save again','ok'); });
    box.querySelector('[data-bn="gatex"]')?.addEventListener('click',()=>{ banners.gate=false; renderBanners(); });
  }
  function simulateDisk(){ loaded['parallel.waveSize']=8; loaded['dashboard.updates']='on-demand'; banners.disk=true; renderBanners(); Rivet.toast('🟣 config changed on disk'); }
  function reloadDisk(){ manifest().forEach(k=>state[k.path]=clone(loaded[k.path])); errorMap={}; banners.disk=false; RT.lastSynced=new Date(); renderBanners(); Rivet.renderSync(); after(); Rivet.toast('⟳ Reloaded from disk','ok'); }
  ...
  return {
    ...
    buildConfig, dirtyPaths, onSearch, doSave, discard, simulateDisk, reloadDisk,
    toggleGate(){ RT.gateLocked=!RT.gateLocked; renderSaveBar(); Rivet.toast(RT.gateLocked?'🛡 GATE-PROTECT armed (tasks in flight)':'🔓 gate released'); },
    toggleServer(){ RT.serverMode=!RT.serverMode; renderBanners(); renderSaveBar(); Rivet.toast(RT.serverMode?'🌐 server mode':'🔒 read-only (file://)'); },
