...
const EVENTS: JournalEvent[] = [
  ...
  ev("2026-06-12T10:05:00Z", "check.run", {
    taskId: "T1",
    result: { ref: "a.test.ts::x", passed: false, at: "2026-06-12T10:05:00Z" },
  }),
  ev("2026-06-12T10:09:00Z", "check.run", {
    taskId: "T1",
    result: { ref: "a.test.ts::x", passed: true, at: "2026-06-12T10:09:00Z" },
  }),
...
describe("deriveTrail — skipped and pending states", () => {
  it("no red run = tddRed skipped; no approval = pending; never-done with attempts = blocked(n)", () => {
    const events: JournalEvent[] = [
      ...
      ev("2026-06-12T11:01:00Z", "check.run", {
        taskId: "T3",
        result: { ref: "b.test.ts::y", passed: true, at: "t" },
      }),
