...
describe("dev-spec-kit init — scaffold the durable .dev-spec-kit/ state", () => {
  it("initializes a fresh project (config, laws, journal, gitignore)", () => {
    const dir = tmpProject();
    const { text } = run(dir, () => runInit({}));
    expect(text).toContain("Initialized dev-spec-kit");
    expect(existsSync(join(dir, ".dev-spec-kit", "config.json"))).toBe(true);
    expect(existsSync(join(dir, ".dev-spec-kit", "laws.md"))).toBe(true);
    expect(existsSync(join(dir, ".dev-spec-kit", "journal.jsonl"))).toBe(true);
    expect(readFileSync(join(dir, ".gitignore"), "utf8")).toContain("graphify-out/");
  });

  it("refuses to clobber an existing config without --force", () => {
    const dir = tmpProject({
      ".dev-spec-kit/config.json": JSON.stringify({ version: 1, project: { name: "tuned" } }),
    });
    const { text } = run(dir, () => runInit({}));
    expect(text).toContain("already initialized");
    // config untouched
    expect(JSON.parse(readFileSync(join(dir, ".dev-spec-kit", "config.json"), "utf8")).project.name).toBe(
      "tuned",
    );
  });
  ...
  it("seeds platform law packs and prints them on a fresh init --platforms", () => {
    ...
    const cfg = JSON.parse(readFileSync(join(dir, ".dev-spec-kit", "config.json"), "utf8"));
  ...
  it("--force overwrites an existing config and re-seeds laws", () => {
    const dir = tmpProject({
      ".dev-spec-kit/config.json": JSON.stringify({ version: 1, project: { name: "old" } }),
    });
  ...
  it("appends gitignore entries when the file exists without a trailing newline", () => {
    ...
    expect(gi).toContain(".dev-spec-kit/cache/");
  });
