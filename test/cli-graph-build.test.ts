...
/** Drives `dev-spec-kit graph build` (refresh:false → no code-graph subprocess) through each gate branch. */
...
describe("dev-spec-kit graph build — the graph + its gates", () => {
  ...
  it("prints the traffic-light summary and refreshes docs for a clean spec (exit 0)", () => {
    const dir = tmpProject({ ".dev-spec-kit/specs/x.md": VALID });
  ...
  it("downgrades an unbound obligation to a warning when the knob is off", () => {
    const dir = tmpProject({
      ".dev-spec-kit/config.json": JSON.stringify({ verify: { everyCriterionNeedsCheck: false } }),
      ".dev-spec-kit/specs/x.md":
  ...
  it("notes a graphify-provider gap without failing (revitify is the bundled fallback)", () => {
    const dir = tmpProject({
      ".dev-spec-kit/config.json": JSON.stringify({ graphify: { provider: "graphify" } }),
      ".dev-spec-kit/specs/x.md": VALID,
    });
    const { text } = run(dir, () => graphBuild({ refresh: false }));
...
describe("dev-spec-kit graph build — drift + satisfied gate packs", () => {
