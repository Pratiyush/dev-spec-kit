...
function project(opts: { inFlight?: boolean; unlock?: boolean } = {}): string {
  const dir = mkdtempSync(join(tmpdir(), "rivet-srv-"));
  ...
  writeFileSync(join(dir, ".rivet", "config.json"), JSON.stringify({ version: 1, project: { name: "srv" } }));
  if (opts.inFlight) {
  ...
  if (opts.unlock) {
    writeFileSync(
      ...
      JSON.stringify({ paths: [".rivet/config.json"], until: new Date(Date.now() + 600_000).toISOString() }),
    );
  }
  return dir;
}
...
describe("REQUIREMENT_COCKPIT-05 — the config save server", () => {
  ...
  it("a valid POST saves config.json and journals governance", async () => {
    ...
    const cfg = JSON.parse(readFileSync(join(dir, ".rivet", "config.json"), "utf8")) as {
      project: { name: string };
  ...
  it("an invalid POST returns field errors and never writes", async () => {
    const dir = project();
    const before = readFileSync(join(dir, ".rivet", "config.json"), "utf8");
    const url = await serve(dir);
    const res = await fetch(`${url}/api/config`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    expect(readFileSync(join(dir, ".rivet", "config.json"), "utf8")).toBe(before);
  });
  ...
  it("in-flight tasks refuse the save with GATE-PROTECT-01 and the unlock hint", async () => {
    ...
    expect(body.unlockHint).toContain("rivet unlock");
    expect(readFileSync(join(dir, ".rivet", "config.json"), "utf8")).toBe(before);
