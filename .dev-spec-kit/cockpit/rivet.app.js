...
(function(){
  ...
  document.addEventListener('keydown', e=>{
    ...
    else if((e.metaKey||e.ctrlKey)&&k==='s'){ e.preventDefault(); if(RT.mode==='config'){ if(RT.serverMode) Config.doSave(); else Rivet.toast('🔒 read-only — run rivet web','warn'); } }
    ...
    else if(e.shiftKey&&k==='d' && RT.mode==='config'){ if(Config.diskActive()) Config.reloadDisk(); }
  ...
  setInterval(() => {
    if (RT.mode === 'config' && Config.hasDirty()) return;
