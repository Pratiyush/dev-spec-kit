...
const Dashboard = (function(){
  ...
  function checkProof(t, c){ const r = t.results && t.results[c]; if (!r) return 'unproven'; if (!r.passed) return 'red'; return r.stale ? 'stale' : 'green'; }  // port: stale evidence renders 🟣, never 🟢
  ...
  function graph(v){
    const g=D().graphHtml;
    const inner=g ? '<div class="card graph-shell"><iframe class="graph-frame" src="'+esc(g)+'" loading="lazy" title="code knowledge graph"></iframe></div>'
      : '<div class="card">'+Rivet.emptyState('🕸️','no graph yet — <code>rivet graph build</code>')+'</div>';  // port: real revitify graph, not the design placeholder
