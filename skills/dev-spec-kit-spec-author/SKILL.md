---
name: dev-spec-kit-spec-author
description: Write high-quality dev-spec-kit specs — Gherkin scenarios (default) or EARS acceptance criteria with @check bindings in .dev-spec-kit/specs/*.md. Use when authoring or revising a spec, converting a ticket/idea into requirements, or when criteria are vague, untestable, or unbound.
---
...
1. **Clarify first** if it's vague (`dev-spec-kit-clarify`): resolve the ambiguity before writing scenarios.
...
5. **Bind + draft** — add the `@check` lines, then `dev-spec-kit spec draft-tests` emits failing stubs and
...
- **The edge-case mandate — 100% behavioral coverage.** For EVERY behavior you MUST name its
  ...
  fails `dev-spec-kit graph build` (`gates.negativeFloor`, on by default) — write the failure Scenario
