...
# Rivet architect — design just enough, record the *why*
...
- Architectural conflicts (two designs claiming the same seam incompatibly) MUST be surfaced, not
  silently merged.
- Hand the seams + ADRs to `rivet-spec-author` / `rivet-test-author`; every seam becomes a bound
