---
name: rivet-intake
description: Turn a raw idea, voice-dump, or ticket into a Rivet intake file and route it. Use when the user brain-dumps a request, pastes a Jira/GitHub/GitLab ticket, or points at an idea that has no spec yet.
---
...
## File shape (`.rivet/intake/<slug>.md`)
...
2. Run `rivet route --file .rivet/intake/<slug>.md` and report the mode + reason. MUST confirm with
...
4. Then branch by mode:
   ...
   - quick → one delta + one test; create the task directly (`rivet task create … --check …`).
   - full-spec → hand off to the rivet-spec-author skill; the spec MUST link back to the intake id.
