---
name: dev-spec-kit-test-author
description: Draft descriptive, behavior-asserting tests from acceptance criteria — run `dev-spec-kit spec draft-tests` to scaffold failing stubs, then flesh each with real assertions covering the 4 edge categories. Use in the TDD red phase, or whenever a criterion is unbound or its test is shallow.
---

# dev-spec-kit test-author — the rule→test→proof loop
...
## Steps
1. **Scaffold:** `dev-spec-kit spec draft-tests` emits a failing stub per unbound criterion (named from the
...
4. **Confirm red, then hand off:** the stub MUST fail before implementation (`dev-spec-kit check run … -x`).
...
- Prefer an executable kind; reach for `kind=judge` (see `dev-spec-kit-judge`) only when the criterion is
